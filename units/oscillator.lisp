...
(defunit osc (unit)
  ((ph :val 0)
   ((init-ph :export :mod) :val 0 :default 0)))

(defunit sine (osc)
  (((freq :export :mod) :default 440 :max 20000 :min 0.05 :step 0.01))
  (let ((v (sin (+ (osc-ph u) (calc-slot (osc-init-ph u))))))
    (incf (osc-ph u) (* (/ (calc-slot (sine-freq u)) +sample-rate+) PI))
    (values v v)))

(defunit tri (osc)
  (((freq :export :mod) :defaut 440 :max 20000 :min 0.05 :step 0.01))
...
(defunit pulse (osc)
  (((freq :export :mod) :defaut 440 :max 20000 :min 0.05 :step 0.01)
   ((duty :export :mod) :default 0.5 :max 1 :min 0 :step 0.01))
